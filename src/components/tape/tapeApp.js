/* 由 scripts/port-cassette.mjs 从 OHM TAPE 原项目 cassette/src/main.js 生成（装配层）*/
...
try {
  ...
  root.innerHTML = '<p style="color:#eee;font:14px/1.6 system-ui;padding:3rem">当前浏览器无法初始化 WebGL，请使用 Chrome / Edge 打开。</p>';
  throw err;
}
...
function whenPlayable() {
  return new Promise((resolve) => {
    ...
    on(audioEl, 'loadedmetadata', ok);
    on(audioEl, 'durationchange', ok);
    on(audioEl, 'error', bad);
...
function applyQuery(camera = true) {
  if (Q.get('ui') === '0') root.querySelector('.ui').style.display = 'none';
  if (Q.get('fps') === '1') {
...
function updateAnnotations() {
  ...
  if (explodedShown !== on) {
    explodedShown = on;
    root.classList.toggle('exploded', on);
...
function setTheme(name, first = false) {
  ...
  for (const b of root.querySelectorAll('#theme button')) {
...
const nextFrame = () => new Promise((r) => requestAnimationFrame(() => later(r, 0)));
...
on(window, 'error', (e) => showError(e.error?.stack || e.message));
on(window, 'unhandledrejection', (e) => showError(e.reason?.stack || e.reason?.message || String(e.reason)));
...
async function boot() {
  await step('正在建立几何体', 12, () => {
    ...
    setDur();
    on(audioEl, 'loadedmetadata', setDur);
    on(audioEl, 'durationchange', setDur);
    on(audioEl, 'error', () => {
...
function applyFocus(key) {
  ...
  root.classList.toggle('focused', !!key);
...
function setFold(on) {
  ...
  if (on && !reduce) foldCamT = later(syncViewShift, 400);
  else syncViewShift();
}
...
function showVolume() {
  ...
  volFlash = later(() => muteBtn.classList.remove('show-vol'), 1100);
}
...
function closeIndex() {
  ...
  later(() => { if (!indexOpen) indexEl.hidden = true; }, 600);
}
...
function applyPref(k) {
  ...
  else if (k === 'keys') root.classList.toggle('keys-off', !prefs.keys);
...
function closeSettings() {
  ...
  later(() => { if (!setOpen) settingsEl.hidden = true; }, 600);
}
...
$('#theme').addEventListener('click', (e) => {
  ...
  if (b) { setTheme(b.dataset.theme); audio.tick(); root.classList.add('moved'); }
});
...
function flashAdd(msg) {
  ...
  addFlash = later(() => addBtn.classList.remove('show-vol'), 1400);
}
function openPicker() {
...
for (const b of root.querySelectorAll('.pk')) {
  b.addEventListener('click', () => {
    ...
    later(() => b.classList.remove('flash'), 240);
  });
}
...
function togglePlay(force) {
  ...
  if (swap.state !== 'idle') {
    if (on) { swap.play = true; root.classList.add('moved'); }
    return;
  }
  st.playing = on;

/* 搬运自 OHM TAPE 原项目 cassette/src/controls.js（保持原样，仅见 scripts/port-cassette.mjs 的补丁）*/
...
export class Orbit {
  constructor(dom, camera, opt = {}) {
    ...
    this.on = opt.on;            // 外部监听注册表（卸载时统一摘除）
    this._ptrs = new Map();
    this._pinch = 0;
  ...
  _bind() {
    ...
    this.on?.(d, 'pointerdown', this._onDown);
    this.on?.(d, 'pointermove', this._onMove);
    this.on?.(window, 'pointerup', this._onUp);
    this.on?.(window, 'pointercancel', this._onUp);
    this.on?.(window, 'wheel', this._onWheel, { passive: false });
    this.on?.(d, 'dblclick', this._onDbl);
    this.on?.(d, 'contextmenu', (e) => e.preventDefault());
  }
